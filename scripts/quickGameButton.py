...
import bsInternal
import bsTeamGame
from bsUI import PlayWindow, AddGameWindow, gSmallUI, \
                 gMedUI, gTitleColor, uiGlobals, gWindowStates
import bsUtils
import settings
...
class SimplePlaylist(object):
    ...
    def pullNext(self):
        if 'map' not in self.settings['settings']:
            settings = dict(map=self.settings['map'],
                            **self.settings['settings'])
        else:
...
class SelectGameWindow(AddGameWindow):
    def __init__(self, transition='inRight'):
        ...
        self._backButton = bs.buttonWidget(
            ...
            color=gWindowsBackColor, textColor=gWindowsBackTextColor,
            size=(165, 70),
            scale=0.75,
            textScale=1.2,
        ...
        self._selectButton = selectButton = bs.buttonWidget(
            ...
            scale=0.75,
            textScale=1.2,
            color=gOkButtonColor,
            textColor=gOkButtonTextColor,
        ...
        bs.textWidget(
            ...
            size=(0, 0),
            scale=1.0,
            text=bs.Lstr(resource='selectGame'),
            hAlign='center',
            color=gTitleColor,
            maxWidth=250,
            vAlign='center')
    ...
    def _refresh(self, selectGetMoreGamesButton=False, selected=None):
        ...
        gameTypes = [gt for gt in bsUtils.getGameTypes() \
            if gt.supportsSessionType(self._editSession._sessionType)]
        ...
        if selectGetMoreGamesButton:
            bs.containerWidget(
                edit=self._column,
                selectedChild=self._getMoreGamesButton,
    ...
    def _add(self):
        ...
        self._selectedGameType.createConfigUI(
    ...
    def onEditGameDone(self, config):
        if config:
            ...
            uiGlobals['mainMenuWindow'] = SelectGameWindow(
...
def newInit(self, *args, **kwargs):
    ...
    self._quickGameButton = bs.buttonWidget(
        parent=self._rootWidget,
        autoSelect=True,
        ...
        scale=1.1,
        textScale=1.2,
        label=bs.Lstr(resource='fastGame'),
        onActivateCall=doQuickGame,
        color=gButtonsColor,
        textColor=gButtonsTextColor)
...
def _save_state(self):
    ...
    if self._rootWidget.getSelectedChild() in swapped:
        gWindowStates[self.__class__.__name__] = \
            swapped[self._rootWidget.getSelectedChild()]
    else:
        print(
            'error saving state for ',
            self.__class__,
            self._rootWidget.getSelectedChild())

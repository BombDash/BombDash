...
class MainMenuActivity(bs.Activity):
    ...
    def onTransitionIn(self):
        ...
        bdVer = '1.7.2'
        bdtext = bdVer + ' Beta 5' \
            if not JRMPmode else 'joi ride madpacke'

        bdtext2 = 'BombDash Modpack' \
            if not JRMPmode else 'joi ride madpacke'
        ...
        def permissionReceiver():
            if not bsInternal._havePermission('storage'):
                ...
                a = bs.gameTimer(
                    1000, bs.Call(permission_receiver), repeat=True
                    )

                bsInternal._fadeScreen(
                    False, time=200, endCall=bs.Call(bs.quit, soft=True)
                    )
        ...
        def getModpackServers():
            if bdUtils.siteAvailabilityCheck('http://bombdash.net'):
                ...
                url = 'http://bombdash.net/other/BombSquad/server/server/internal.php'

                req = json.loads(urllib2.urlopen(
                    urllib2.Request(url), json.dumps(val)
                    ).read().decode('utf-8'))
        ...
        if startEvent == 1:
            bs.playMusic('intro')
            bs.screenMessage(
                bs.Lstr(resource='musicText').evaluate()+'The XX - Intro'
                )
            ...
            bs.screenMessage(
                bs.Lstr(resource='musicText').evaluate()+'Portal 2 OST - Turrets opera'
                )

            self.turretsArrayNode = bs.NodeActor(bs.newNode('terrain', attrs={
                'model': turretsArray \
                    if bs.getEnvironment()['platform'] != 'android' else turretsArrayLow,
                'reflection': 'powerup',
                'reflectionScale': [0.9],
            ...
            bs.screenMessage(
                bs.Lstr(resource='musicText').evaluate()+'Zack Hemsey - The Way (Instrumental)')
            ...
            bs.gameTimer(
                150 if not bs.getEnvironment()['platform'] == 'android' else 250,
                bs.Call(spawnDirt), repeat=True
                )
            ...
            for i in range(30 if not bs.getEnvironment()['platform'] == 'android' else 15):
            ...
            def reBoom():
                ...
                bsUtils.animate(
                    ...
                    {0: 1, 100: 1.05, 110: 0.3, 120: 1,
                     800: 1.1, 900: 1, 910: 0.6, 930: 1.1,
                     1000: 1, 1500: 1, 1510: 0, 1520: 1,
                     3000: 0.6, 3300: 1, 4000: 1.1}, True
                    )
            ...
            bs.screenMessage(
                bs.Lstr(resource='musicText').evaluate()+'Zack Hemsey - Mind Heist'
                )
            ...
            bs.gameTimer(
                10 if bs.getEnvironment()['platform'] == 'android' else 10,
                dropBGD, True
                )
            ...
            def dropB():
                    ...
                    bs.gameTimer(
                        500+int(random.random()*300),
                        bs.Call(expl)
                        )

            bs.gameTimer(
                800+int(random.random()*350),
                bs.Call(dropB),
                repeat=True
                )
            ...
            bs.screenMessage(
                bs.Lstr(resource='musicText').evaluate()+'Undertale - Waterfall Remix'
                )
            ...
            def Lights1():
                ...
                def deleteLights():
                    if self.light1.exists(): self.light1.delete()
            ...
            def Lights2():
                ...
                def deleteLights():
                    if self.light2.exists(): self.light2.delete()
            ...
            def Lights3():
                ...
                def deleteLights():
                    if self.light3.exists(): self.light3.delete()
            ...
            def StrobeTimer(on=True):
                if on:
                    ...
                    self.ltstrobe = None
                    if self.light4.exists(): self.light4.delete()
            ...
            def drop():
                self.jumping = True
                LightsSlowshowTimer(on=False) #off 
                LightsFastshowTimer()
                StrobeTimer()
                def jump():
                    s = int(random.random() * len(self._spazArray))
                    self._spazArray[s].onJumpPress()
                    self._spazArray[s].onJumpRelease()
                    if self.jumping == True and not s == 40:
                        bs.gameTimer(50, bs.Call(jump))
            ...
            bs.screenMessage(
                bs.Lstr(resource='musicText').evaluate()+'Georgi Kay - In My Mind (Axwell Mix)'
                )
            ...
            bs.screenMessage(
                bs.Lstr(resource='musicText').evaluate()+'Basshunter - Dota'
                )
            ...
            bs.screenMessage(bs.Lstr(resource='musicText').evaluate()+'Zack Hemsey - Redemption')
        ...
        if not startEvent in [4, 5, 6, 8, 11, 13, 14]:

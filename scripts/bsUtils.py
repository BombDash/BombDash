...
gHalloween = datetime.datetime.now().month == 10 \
    and datetime.datetime.now().day > 29 \
    and datetime.datetime.now().day != 1

gBDBirthday = datetime.datetime.now().month == 11 \
    and datetime.datetime.now().day == 6

gNewYear = (datetime.datetime.now().month == 12 \
    and datetime.datetime.now().day > 6) \
    or (datetime.datetime.now().month == 1 \
    and datetime.datetime.now().day < 16)
...
gThemeEngineLimiter = 0
gThemeCriticalErrors = {}
gThemeNotCompatibilityCheck = []
...
class ThemeEngine(object):
    ...
    """
    def __init__(self):
        self.themeValues = None
        self.themeValuesDefault = None
        self.errors = []
    ...
    def compatibilityCheck(self):
        ...
        for x in scripts:
            if 'theme' in x and '.pyc' in x:
                thName = x.replace('.pyc', '')
                th = __import__(thName)
                thVal = dir(th)[5:]
                for i in thVal:
                    if i not in self.themeValuesDefault and not i.startswith('theme'):
                        now = datetime.datetime.now()
                        with open(self.path+'/themesErrors.txt', 'a+') as f:
                            f.write(
                                now.strftime('[%Y-%m-%d %H:%M:%S]')+\
                                ' Value "'+i+'" in theme "'+thName+'", '+\
                                'not needed in the current version of the theme engine! '+\
                                'Delete it.\n')

                        if thName not in gThemeNotCompatibilityCheck:
                            gThemeNotCompatibilityCheck.append(thName)

                for i in self.themeValuesDefault:
                    if i not in thVal and not i.startswith('theme'):
                        now = datetime.datetime.now()
                        with open(self.path+'/themesErrors.txt', 'a+') as f:
                            f.write(
                                now.strftime('[%Y-%m-%d %H:%M:%S]')+\
                                ' Value "'+i+'" in theme "'+thName+'", '+\
                                'is missing, although it is necessary in this version '+\
                                'of the engine! Add this value.\n')

                        if thName not in gThemeNotCompatibilityCheck:
                            gThemeNotCompatibilityCheck.append(thName)

    def criticalErrorHandling(self):
        if gThemeCriticalErrors:
            for n in gThemeCriticalErrors:
                now = datetime.datetime.now()
                with open(self.path+'/themesErrors.txt', 'a+') as f:
                    f.write(
                        now.strftime('[%Y-%m-%d %H:%M:%S]')+\
                        ' Critical error in "'+n+'" theme!'+\
                        ' | Line: '+gThemeCriticalErrors[n][0]+\
                        ' | Code: '+gThemeCriticalErrors[n][1])
    ...
    def writeError(self):
        for i in self.errors:
            ...
            with open(self.path+'/themesErrors.txt', 'a+') as f:
                f.write(
                    now.strftime('[%Y-%m-%d %H:%M:%S]')+\
                    ' Error in current theme, in "'+str(i)+'" value!\n')

    def checkErrors(self):
        for i in self.themeValues:
            try:
                if self.themeValues[i] is None: pass
                elif type(self.themeValues[i]) is str: pass
                elif type(self.themeValues[i]) is tuple:
                    for x in self.themeValues[i]:
                        if type(x) is float or type(x) is int: pass
                        else:
                            self.errors.append(i)
                            self.themeValues[i] = self.themeValuesDefault[i]
            except:
                self.errors.append(i)
                self.themeValues[i] = self.themeValuesDefault[i]
        else:
            GetThemeValues.values = self.themeValues

    def getThemeValues(self):
        if self.currentTheme == 'Default':
            self.themeValues = self.themeValuesDefault
        else:
            self.themeValues = {
                'themeName': self.currentTheme.themeName,
                'themeAuthor': self.currentTheme.themeAuthor,
                'themeButtonColor': self.currentTheme.themeButtonColor,
                'themeButtonTextColor': self.currentTheme.themeButtonTextColor,
                'gTextColor': self.currentTheme.gTextColor,
                'gExtraTextColor': self.currentTheme.gExtraTextColor,
                'gGoodTextColor': self.currentTheme.gGoodTextColor,
                'gBadTextColor': self.currentTheme.gBadTextColor,
                'gTitleColor': self.currentTheme.gTitleColor,
                'gHeadingColor': self.currentTheme.gHeadingColor,
                'gInfoTextColor': self.currentTheme.gInfoTextColor,
                'gWindowsColor': self.currentTheme.gWindowsColor,
                'gPopupWindowColor': self.currentTheme.gPopupWindowColor,
                'gPopupWindowInactiveColor': self.currentTheme.gPopupWindowInactiveColor,
                'gPopupWindowActiveColor': self.currentTheme.gPopupWindowActiveColor,
                'gPopupWindowScrollColor': self.currentTheme.gPopupWindowScrollColor,
                'gWindowsScrollColor': self.currentTheme.gWindowsScrollColor,
                'gWindowsBackColor': self.currentTheme.gWindowsBackColor,
                'gWindowsBackTextColor': self.currentTheme.gWindowsBackTextColor,
                'gWindowsCrossoutColor': self.currentTheme.gWindowsCrossoutColor,
                'gButtonsColor': self.currentTheme.gButtonsColor,
                'gButtonsTextColor': self.currentTheme.gButtonsTextColor,
                'gInversionColor': self.currentTheme.gInversionColor,
                'gInversionTextColor': self.currentTheme.gInversionTextColor,
                'gInversionTextColor2': self.currentTheme.gInversionTextColor2,
                'gInversionCrossoutColor': self.currentTheme.gInversionCrossoutColor,
                'gCheckBoxColor': self.currentTheme.gCheckBoxColor,
                'gCheckBoxTextColor': self.currentTheme.gCheckBoxTextColor,
                'gOkButtonColor': self.currentTheme.gOkButtonColor,
                'gOkButtonTextColor': self.currentTheme.gOkButtonTextColor,
                'gCancelButtonColor': self.currentTheme.gCancelButtonColor,
                'gCancelButtonTextColor': self.currentTheme.gCancelButtonTextColor,
                'gWindowsTabsActiveColor': self.currentTheme.gWindowsTabsActiveColor,
                'gWindowsTabsActiveTextColor': self.currentTheme.gWindowsTabsActiveTextColor,
                'gWindowsTabsColor': self.currentTheme.gWindowsTabsColor,
                'gWindowsTabsTextColor': self.currentTheme.gWindowsTabsTextColor,
                'gLobbyTextColor': self.currentTheme.gLobbyTextColor,
                'gLobbyScoreTextColor': self.currentTheme.gLobbyScoreTextColor,
                'gLobbyScoreTextColor2': self.currentTheme.gLobbyScoreTextColor2,
                'gLobbyScoreLeadersTextColor': self.currentTheme.gLobbyScoreLeadersTextColor,
                'gHackBackgroundTextColor': self.currentTheme.gHackBackgroundTextColor,
                'gBackground': self.currentTheme.gBackground,
                'gBackgroundColor': self.currentTheme.gBackgroundColor,
                'menuImgMenuColor': self.currentTheme.menuImgMenuColor,
                'menuGameNameMenuColor': self.currentTheme.menuGameNameMenuColor,
                'menuBDMPMenuColor': self.currentTheme.menuBDMPMenuColor,
                'menuEricFroemlingMenuColor': self.currentTheme.menuEricFroemlingMenuColor,
                'menuBDMPVersionMenuColor': self.currentTheme.menuBDMPVersionMenuColor,
                'menuStoreNotificationColor': self.currentTheme.menuStoreNotificationColor,
                'menuStoreNotificationTextColor': self.currentTheme.menuStoreNotificationTextColor,
                'menuThemeNotificationColor': self.currentTheme.menuThemeNotificationColor,
                'menuThemeNotificationTextColor': self.currentTheme.menuThemeNotificationTextColor,
                'menuResumeButtonColor': self.currentTheme.menuResumeButtonColor,
                'menuResumeButtonTextColor': self.currentTheme.menuResumeButtonTextColor,
                'menuInBattleButtonColor': self.currentTheme.menuInBattleButtonColor,
                'menuInBattleButtonTextColor': self.currentTheme.menuInBattleButtonTextColor,
                'menuGatherButtonColor': self.currentTheme.menuGatherButtonColor,
                'menuGatherButtonTextColor': self.currentTheme.menuGatherButtonTextColor,
                'menuGCButtonColor': self.currentTheme.menuGCButtonColor,
                'menuGCButtonTextColor': self.currentTheme.menuGCButtonTextColor,
                'menuThemesButtonColor': self.currentTheme.menuThemesButtonColor,
                'menuThemesButtonTextColor': self.currentTheme.menuThemesButtonTextColor,
                'menuExtraButtonsButtonColor': self.currentTheme.menuExtraButtonsButtonColor,
                'menuExtraButtonsButtonTextColor': self.currentTheme.menuExtraButtonsButtonTextColor,
                'menuSettingsButtonColor': self.currentTheme.menuSettingsButtonColor,
                'menuSettingsButtonTextColor': self.currentTheme.menuSettingsButtonTextColor,
                'menuStoreButtonColor': self.currentTheme.menuStoreButtonColor,
                'menuStoreButtonTextColor': self.currentTheme.menuStoreButtonTextColor,
                'menuWatchButtonColor': self.currentTheme.menuWatchButtonColor,
                'menuWatchButtonTextColor': self.currentTheme.menuWatchButtonTextColor,
                'menuGroupButtonColor': self.currentTheme.menuGroupButtonColor,
                'menuGroupButtonTextColor': self.currentTheme.menuGroupButtonTextColor,
                'menuSiteButtonColor': self.currentTheme.menuSiteButtonColor,
                'menuSiteButtonTextColor': self.currentTheme.menuSiteButtonTextColor,
                'menuCreditsButtonColor': self.currentTheme.menuCreditsButtonColor,
                'menuCreditsButtonTextColor': self.currentTheme.menuCreditsButtonTextColor,
                'menuHowToPlayButtonColor': self.currentTheme.menuHowToPlayButtonColor,
                'menuHowToPlayButtonTextColor': self.currentTheme.menuHowToPlayButtonTextColor,
                'menuAboutBDButtonColor': self.currentTheme.menuAboutBDButtonColor,
                'menuAboutBDButtonTextColor': self.currentTheme.menuAboutBDButtonTextColor,
                'menuStatsButtonColor': self.currentTheme.menuStatsButtonColor,
                'menuStatsButtonTextColor': self.currentTheme.menuStatsButtonTextColor,
                'menuQuitButtonColor': self.currentTheme.menuQuitButtonColor,
                'menuQuitButtonTextColor': self.currentTheme.menuQuitButtonTextColor,
                'helpWindowParagraphColor': self.currentTheme.helpWindowParagraphColor,
                'helpWindowHeaderColor': self.currentTheme.helpWindowHeaderColor,
                'helpWindowHeader2Color': self.currentTheme.helpWindowHeader2Color,
                'watchWindowLastGameTextColor': self.currentTheme.watchWindowLastGameTextColor,
                'multiplayerStatsButtonColor': self.currentTheme.multiplayerStatsButtonColor,
                'multiplayerStatsButtonTextColor': self.currentTheme.multiplayerStatsButtonTextColor,
                'multiplayerServerNameColor': self.currentTheme.multiplayerServerNameColor,
                'multiplayerPingServerColor': self.currentTheme.multiplayerPingServerColor,
                'multiplayerPingServerVeryGoodColor': self.currentTheme.multiplayerPingServerVeryGoodColor,
                'multiplayerPingServerGoodColor': self.currentTheme.multiplayerPingServerGoodColor,
                'multiplayerPingServerMedColor': self.currentTheme.multiplayerPingServerMedColor,
                'multiplayerSizeColor': self.currentTheme.multiplayerSizeColor,
                'multiplayerBombDashServersTextColor': self.currentTheme.multiplayerBombDashServersTextColor,
                'storeWindowButtonPurchasedColor': self.currentTheme.storeWindowButtonPurchasedColor,
                'storeWindowButtonPurchasedTextColor': self.currentTheme.storeWindowButtonPurchasedTextColor,
                'storeWindowButtonNotPurchasedColor': self.currentTheme.storeWindowButtonNotPurchasedColor,
                'storeWindowButtonNotPurchasedTextColor': self.currentTheme.storeWindowButtonNotPurchasedTextColor,
                'storeWindowButtonMaybePurchasedColor': self.currentTheme.storeWindowButtonMaybePurchasedColor,
                'storeWindowButtonMaybePurchasedTextColor': self.currentTheme.storeWindowButtonMaybePurchasedTextColor,
                'storeWindowCheckMarkColor': self.currentTheme.storeWindowCheckMarkColor,
                'getTicketsWindowTextColor': self.currentTheme.getTicketsWindowTextColor,
                'getTicketsFreeWindowTextColor': self.currentTheme.getTicketsFreeWindowTextColor,
                'powerRankingButtonColor': self.currentTheme.powerRankingButtonColor,
                'powerRankingButtonTextColor': self.currentTheme.powerRankingButtonTextColor,
                'storeButtonColor': self.currentTheme.storeButtonColor,
                'storeButtonTextColor': self.currentTheme.storeButtonTextColor,
                'campaignEasyButtonColor': self.currentTheme.campaignEasyButtonColor,
                'campaignHardButtonColor': self.currentTheme.campaignHardButtonColor,
                'campaignUnButtonColor': self.currentTheme.campaignUnButtonColor,
                'campaignOnButtonTextColor': self.currentTheme.campaignOnButtonTextColor,
                'campaignUnButtonTextColor': self.currentTheme.campaignUnButtonTextColor,
                'achievementsCompleteNumbersColor': self.currentTheme.achievementsCompleteNumbersColor,
                'achievementsNotCompleteNumbersColor': self.currentTheme.achievementsNotCompleteNumbersColor,
                'achievementsCompleteNamesColor': self.currentTheme.achievementsCompleteNamesColor,
                'achievementsNotCompleteNamesColor': self.currentTheme.achievementsNotCompleteNamesColor,
                'achievementsCompleteDescriptionColor': self.currentTheme.achievementsCompleteDescriptionColor,
                'achievementsNotCompleteDescriptionColor': self.currentTheme.achievementsNotCompleteDescriptionColor,
                'achievementsCompletePointsColor': self.currentTheme.achievementsCompletePointsColor,
                'achievementsNotCompletePointsColor': self.currentTheme.achievementsNotCompletePointsColor,
                'trophiesHaveNumberColor': self.currentTheme.trophiesHaveNumberColor,
                'trophiesNotHaveNumberColor': self.currentTheme.trophiesNotHaveNumberColor,
                'trophiesHaveMultColor': self.currentTheme.trophiesHaveMultColor,
                'trophiesNotHaveMultColor': self.currentTheme.trophiesNotHaveMultColor,
                'trophiesHavePointsColor': self.currentTheme.trophiesHavePointsColor,
                'trophiesNotHavePointsColor': self.currentTheme.trophiesNotHavePointsColor,
                'manualBadColor': self.currentTheme.manualBadColor,
                'manualGoodColor': self.currentTheme.manualGoodColor
                }

        self.checkErrors()

    def getDefaultTheme(self):
        if self.themeValuesDefault is None:
            self.themeValuesDefault = {
                'gTextColor': (1, 1, 1),
                ...
                'gInversionColor': (0.345, 0.321, 0.423),
                ...
                'gHackBackgroundTextColor': (0.5, 1.0, 0.5, 0.18),
                'gBackground': False,
                'gBackgroundColor': (1, 1, 1),
                'menuImgMenuColor': (0.01, 0.01, 0.01),
                'menuGameNameMenuColor': (1.2, 1.15, 1.15),
                'menuBDMPMenuColor': (1, 1, 1),
                ...
                'menuGroupButtonColor': (0.101946, 0.643044, 0.999855),
                'menuGroupButtonTextColor': (1, 1, 1),
                'menuSiteButtonColor': (0.2, 0.74, 0.40),
                ...
                'menuQuitButtonColor': (1.0, 0.35, 0.35),
                ...
                'multiplayerStatsButtonColor': (0.3, 0.6, 0.94),
                'multiplayerStatsButtonTextColor': (1, 1, 1),
                'multiplayerServerNameColor': "(1, 1, 1, 0.3 if party['ping'] is None else 1.0)",
                'multiplayerPingServerColor': (0.1, 0.1, 0.1),
                'multiplayerPingServerVeryGoodColor': (0, 1, 0),
                'multiplayerPingServerGoodColor': (1, 1, 0),
                'multiplayerPingServerMedColor': (1, 0, 0),
                'multiplayerSizeColor': (0.8, 0.8, 0.8),
                'multiplayerBombDashServersTextColor': (0.0, 1.0, 0.0),
                ...
                'getTicketsWindowTextColor': '(0.7, 0.9, 0.7, 1.0 if enabled else 0.2)',
                'getTicketsFreeWindowTextColor': (1, 1, 0),
...
themeEngineClass = ThemeEngine()

# import of values from theme...
themeValues = GetThemeValues.values
...
def addStats(name):
    ...
    """
    if bs.getActivity().getName() != 'MainMenuActivity':
        try:
            s = bs.getConfig()['BombDash Stats'].get(str(name), 0)
        except:
            s = 0

        s += 1
        bs.getConfig()['BombDash Stats'][str(name)] = s
        bs.writeConfig()
        ...
        for name in dirlist:
            try:
                ...
                if name.startswith('theme'):
                    moduleName = name.replace('.py', '')
                    gThemeCriticalErrors[moduleName] = [str(e[1][1]), str(e[1][3])]
        ...
        # handle via internal music...
            ...
            if musicType is not None:
                ...
                if musicType == 'Menu':
                    ...
                    volume = 4.0
                elif musicType == 'TheNether':
                    filename = 'DarrenKorbTheMansersDilemma'
                    volume = 4.0
                elif musicType == 'MineWindbagMine':
                    filename = 'BastionOST_MineWindbagMine'
                    volume = 4.0
                elif musicType == 'menuScene4':
                    filename = 'TheWay'
                    volume = 4.0
                elif musicType == 'turretOpera':
                    filename = 'turretOpera'
                    volume = 3.0
                elif musicType == 'ThisEndsNow':
                    filename = 'ThisEndsNow_ReallySlowMotion'
                    volume = 3.0
                elif musicType == 'Run':
                    filename = 'run'
                    volume = 3.0
                    ...
                    filename = 'waterWaves'
                    volume = 4.0
                elif musicType == 'doomsday':
                    filename = 'doomsday'
                    volume = 5.0
                elif musicType == 'intro':
                    filename = 'intro'
                    volume = 3.0
                elif musicType == 'Redemption':
                    filename = 'ZackHemseyRedemption'
                    volume = 5.0
                elif musicType == 'MindHeist':
                    filename = 'ZackHemseyMindHeist'
                    volume = 4.0
                elif musicType == 'JingleBells':
                    filename = 'JingleBells'
                    volume = 3.0
                elif musicType == 'WaterFall':
                    filename = 'waterfallrmx'
                    volume = 3.0
                elif musicType == 'InMyMind':
                    filename = 'InMyMind'
                    volume = 3.0
...
class Background(bsGame.Actor):
    ...
    def __init__(self, fadeTime=500, startFaded=False, showLogo=False,
        ...
        with bs.Context(session):
            ...
            elif gBackground and customBackground:
                self.node = bs.newNode('image', delegate=self, attrs={
                    'fillScreen':True,
                    'texture':bs.getTexture('tipTopBGColor'),
                    'tiltTranslate':-0.3,
                    'hasAlphaChannel':False,
                    'color':gBackgroundColor})
            elif gBackground and not customBackground:
                self.node = bs.newNode('image', delegate=self, attrs={
                    'fillScreen':True,
                    'texture':bs.getTexture('bg'),
                    'tiltTranslate':-0.3,
            ...
            if showLogo:
                logoTexture = bs.getTexture('logoBD')
...
class TipsText(bsGame.Actor):
    ...
    def __init__(self, offsY=100):
        ...
        self._changeTimer = bs.Timer(self._messageDuration+self._messageSpacing,
                                     ...
                                     repeat=True)
        self._combine = bs.newNode("combine", owner=self.node, attrs={
            'input0':gLobbyTextColor[0],
            'input1':gLobbyTextColor[1],
            'input2':gLobbyTextColor[1],
            'size':4})

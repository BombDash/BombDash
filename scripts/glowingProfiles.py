...
class Chooser(object):
    ...
    def __init__(self, vPos, player, lobby):
        ...
        self.ready = False
        self._textNode = bs.newNode('text', delegate=self, attrs={
        ...
        self.icon = bs.newNode('image', owner=self._textNode, attrs={
    ...
    def getGlowingColors(self):
        try:
            ...
            bs.screenMessage('Error initializing glowing code!', color=(1, 0.3, 0))
    ...
    def _setReady(self, ready):
        ...
        if not ready:
            ...
            if settings.cmdNew:
                if self._player.get_account_id() in bs.getConfig()['BombDash Privilege']['bans']:
                    self.ban = True
                ...
                if not self.ban:
                    ...
                    bsInternal._disconnectClient(self._player.getInputDevice().getClientID())
            else:
                bs.getSession().handleMessage(PlayerReadyMessage(self))
        ...
        if isinstance(m, ChangeMessage):
            ...
            if m.what == 'team':
                ...
                if len(self.profileNames) == 1:
                    ...
                    self.profileIndex = ((self.profileIndex + m.value)
                ...
                self.characterIndex = ((self.characterIndex + m.value)
                                       % len(self.characterNames))
                self._updateText()
                self._updateIcon()
...
import bsLobby
bsLobby.Lobby = Lobby
bsLobby.PlayerReadyMessage = PlayerReadyMessage
bsLobby.ChangeMessage = ChangeMessage

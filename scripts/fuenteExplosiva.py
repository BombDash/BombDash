...
def bsGetGames():
    return [FuenteExplosiva]

class FuenteExplosiva(bs.TeamGameActivity):

    @classmethod
    def getName(cls):
        return 'Fuente explosiva'
    ...
    def _dropBombCluster(self):
        ...
        for i in range(random.randrange(1,3)):
            ...
            pos = (-20+40*random.random(),0.01,-10+20*random.random())
            vel = ((0+random.random()*0) * (0 if pos[0] > 0 else 0), 0,0)
    ...
    def _decrementMeteorTime(self):
        self._meteorTime = max(1,int(self._meteorTime*0.7))

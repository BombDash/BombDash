...
class BoxingOfTheHillGame(bs.TeamGameActivity):
    ...
    def getSettings(cls,sessionType):
        return [("Hold Time",{'minValue':10,'default':30,'increment':10}),
                ...
                ("Respawn Times",{'choices':[('Shorter',0.25),('Short',0.5),('Normal',1.0),('Long',2.0),('Longer',4.0)],'default':1.0}),("Change day to night",{'default':True})]
    ...
    def onBegin(self):
        ...
        if self.settings["Change day to night"]:
            ...
            if self._isSlowMotion == False:
                bs.animate(c,'input0',{0:tint[0],60000:0.6,120000:0.5 if bs.getEnvironment()['platform'] == 'android' else 0.2 ,180000:0.6,240000:tint[0]},loop=True)
                bs.animate(c,'input1',{0:tint[1],60000:0.3,120000:0.7 if bs.getEnvironment()['platform'] == 'android' else 0.2 ,180000:0.3,240000:tint[1]},loop=True)
                bs.animate(c,'input2',{0:tint[2],60000:0.3,120000:1 if bs.getEnvironment()['platform'] == 'android' else 0.6 ,180000:0.3,240000:tint[2]},loop=True)
            else:
                bs.animate(c,'input0',{0:tint[0],30000:0.6,60000:0.5 if bs.getEnvironment()['platform'] == 'android' else 0.2,90000:0.6,120000:tint[0]},loop=True)
                bs.animate(c,'input1',{0:tint[1],30000:0.3,60000:0.7 if bs.getEnvironment()['platform'] == 'android' else 0.2,90000:0.3,120000:tint[1]},loop=True)
                bs.animate(c,'input2',{0:tint[2],30000:0.3,60000:1 if bs.getEnvironment()['platform'] == 'android' else 0.6 ,90000:0.3,120000:tint[2]},loop=True)
            def superkostil():
